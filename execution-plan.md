...
| W7–W8 | G3-P1 因果 GPU admission + selective migration validation/test → 公平 two-tier baselines | G3 | 已实现 full / selective-migrate-only / always-migrate 三层消融、成本/历史证据 doorkeeper；future index 与在线策略物理隔离；task-grouped validation 冻结含 cold-start cost ratio 的四个参数；2 GiB/c=4 held-out protocol report；two-tier LRU/GDSF/lookahead reference；主 cell closed-loop TTFT/throughput/goodput | validation 无有效配置则重审 proxy 或记录不支持；`PROTOCOL-INCOMPLETE` 不切路线；仅协议完整且 closed-loop 正式门槛失败才触发路线 A No-Go |
